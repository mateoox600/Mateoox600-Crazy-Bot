        ...
        if (message.getContentRaw().contains("Encode or Decode (E/D):")) {
            ...
            if (e.getReactionEmote().getName().equals("\uD83C\uDDEA")) {
                ...
                try {
                    ...
                    message.delete().queue(msg -> e.getChannel().sendMessage("<@" + message.getContentRaw().split(" ")[0].substring(2, message.getContentRaw().split(" ")[0].length() - 1) + "> ```" + output.toString() + "```").queue(msg1 -> MCB.reactionsEventMessage.remove(e.getMessageId())));
